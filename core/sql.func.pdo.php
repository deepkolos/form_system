...
//链接
function Connect(){
    $link = new PDO('mysql:host='.DB_HOST.';charset=utf8;dbname='.DB_NAME,DB_USER,DB_PSWD);
    $link->exec('set names utf8');//与上面的不冲突~~
}
...
function insert($table,$array){
    ...
    //感觉自己这样封装以后会出现事务问题,希望不会,不行~~如果这样改的话,
    //所有sql链接都需要改变,因为数据直接是返回数组而不是一行一行返回
...
function lastMeeting(){
    $getLastMeeting = queryLast("meetingoverview",array("id","time","islock"),"id");
    $last_meeting = mysql_fetch_assoc($getLastMeeting);
    return $last_meeting;
}
...
function newMeeting(){
    //检查数据库是否为空
    $last_meeting = queryLast("meetingoverview",array("id","time","islock"),"id");
    $result = mysql_fetch_assoc($last_meeting);

    //新建记录前,把前一个上锁
    if($result)
        update("meetingoverview",array("islock"=>"1"),array("id"=>$result["id"]));
    //说明没有没有记录||超过3天,新建一次会议
    insert("meetingoverview",array(
        "time"   =>"CURDATE()",
        "islock" =>"0"
    ));
    //新建表 sample:meeting_{id}
    $tableName = prefixTable.(($result)?(int)$result["id"]+1:1);
    newTable( $tableName,"
        id int,
        state int,
        PRIMARY KEY (id)
